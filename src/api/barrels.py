...
def post_deliver_barrels(barrels_delivered: list[Barrel]):
    ...
    with db.engine.begin() as connection:
        for barrel in barrels_delivered:
            ...
            # add cash_ledger
            add_cash_ledger_sql = f"INSERT INTO cash_ledger(type,description,amount) VALUES ('withdrawl',:description,:amount)"
            parameters = {
                "description":f"Purchased {barrel.quantity} amount of {barrel.sku} for ${total_cost} at ${barrel.price} per barrel",
                "amount":-total_cost
            }
            ...
            # update barrel table
            update_barrel_sql = f"INSERT INTO barrel_ledger (type, sku, amount, description) VALUES ('Delivered', :sku, :amount, :description)"
            parameters = {
                "sku":f"{color}_barrel",
                "amount":barrel.quantity*barrel.ml_per_barrel,
                "description":f"Delivered {barrel.quantity} {color}_barrel"
            }
            result = connection.execute(statement=sqlalchemy.text(update_barrel_sql),parameters=parameters)
    return "OK"
...
def get_wholesale_purchase_plan(wholesale_catalog: list[Barrel]):
    ...
    with db.engine.begin() as connection:
        ...
        parameters = {
            "barrel_catalog_json":barrel_catalog_json
        }
